...
real_diff_dense = np.abs(dense-dense)
...
Contour_X = Sample_X
Contour_Y = Sample_Y
Contour_Z = np.reshape(real_diff_dense, Contour_X.shape)
...
contourf_set = axis.contourf(Contour_X, Contour_Y, Contour_Z, alpha=0.7, cmap='Blues', levels=np.linspace(0, 5, 6))
...
plt.title(str(sec2time(test_sec)) + "密度分布真实无误差", fontsize='7')


# 预测图-----------------------------------------------
for Link_Num in range(2, 10):
    # 加载预测SVR数据
    load_name = './SVRPredictData/all_y_hat_svr_' + str(Link_Num) + '.npz'
    data = np.load(load_name)
    ALL_y_hat = data['all_y_hat']

    ax = fig.add_subplot(3, 3, Link_Num)
    plt.sca(ax)
    ...
    diff_dense = np.abs(dense-predict_dense)
    ...
    plt.title(str(sec2time(test_sec)) + "预测误差分布，预测窗口长：" + str(Link_Num), fontsize='7')
...
# 图片存储在HotSpotPlot目录
plt_name = './PredictHotSpotSVR/HotSpot_Real_Predict_Diff'+str(test_sec)+'_2-9_update'
