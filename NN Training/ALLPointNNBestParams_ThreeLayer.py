...
for Hidden_Point1 in range(1, 5):
    # 进度
    print(Hidden_Point1, '/25')
    for Hidden_Point2 in range(5, 25):
        ...
        # 评价指标数据汇总
        Sum_eva = np.array([[Hidden_Point1, Hidden_Point2, np.mean(MSE_ALL), np.mean(MAE_ALL), np.mean(R2_ALL)]])
        Sum_eva = pd.DataFrame(data=Sum_eva)
        save_data_path = 'NNPredictData/Three_layer_Sum_Params.csv'
