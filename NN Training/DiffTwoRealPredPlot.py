...
predict_dense = ALL_y_hat[:, Plot_Time, Link_Num]

print(dense.shape, predict_dense.shape)

diff_dense = dense-predict_dense
diff_dense = np.abs(diff_dense)
print(diff_dense, diff_dense.shape)
...
Contour_X = Sample_X
Contour_Y = Sample_Y
Contour_Z = np.reshape(diff_dense, Contour_X.shape)
...
# cm = plt.cm.get_cmap('RdYlBu_r') # RdYlBu_r末尾加r表示颜色取反
# contourf_set = axis.contourf(Contour_X, Contour_Y, Contour_Z, levels=np.linspace(0, Contour_Z.max(), 12), alpha=0.7, cmap='Reds')
contourf_set = axis.contourf(Contour_X, Contour_Y, Contour_Z, alpha=0.7, cmap='Blues',levels=levels)
...
plt.title(str(sec2time(test_sec)) + "时刻MLP密度预测误差程度分布情况", fontsize=10)


# 图片存储在HotSpotPlot目录
plt_name = './PredictHotSpotNN/HotSpot_Real_Predict_MLP_Diff'+str(test_sec)+'_'+str(Link_Num)+'update'
# plt_name = './PredictHotSpotNN/HotSpot_Real_Predict_LSTM_Diff'+str(test_sec)+'_'+str(Link_Num)+'update'
# plt_name = './PredictHotSpotSVR/HotSpot_Real_Predict_SVR_Diff'+str(test_sec)+'_'+str(Link_Num)+'update'
