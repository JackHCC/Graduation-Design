...
# 加载数据
data = np.load('./TrainData/all_point_data_20.npz')
...
for point in range(Point_Num):
    ...
    # max_iter=200，最大迭代次数
    MSE_Select = []
    for Hidden_Point in range(5, 25):
        ...
        MSE_Select = np.append(MSE_Select, MSE_Test)

    # 最佳神经元个数
    Best_Hidden_Point_MSE = np.argmin(MSE_Select) + 1
    Best_Hidden_ALL = np.append(Best_Hidden_ALL, Best_Hidden_Point_MSE)

    clf = MLPRegressor(solver='sgd', alpha=1e-4, learning_rate_init=0.01, hidden_layer_sizes=(Best_Hidden_Point_MSE, 1),
                       activation='relu',
                       max_iter=1000, random_state=1)
    clf.fit(X_train_data, Y_train_data)
...
print(Best_Hidden_ALL)
print("Hidden mean:", np.mean(Best_Hidden_ALL))
# counts = np.bincount(Best_Hidden_ALL)
# #返回众数
# print("Hidden Count:", np.argmax(counts))
